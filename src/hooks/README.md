...
#### 2026-08-31 — 후속 prefetch가 실행되지 않던 문제 수정

- **발견된 문제**: 즉시 prefetch 요청이 1.5초 이상 걸리면 타이머로 시작한 두 번째 요청과 캐릭터 탭의 focus refetch가 아직 진행 중인 동일 쿼리 요청으로 합쳐졌습니다. 그 결과 서버 반영 전의 첫 응답만 캐시에 남고 후속 네트워크 요청은 사라져, 다른 탭에 갔다 돌아와 다시 refetch해야 출석·포인트가 갱신됐습니다.
