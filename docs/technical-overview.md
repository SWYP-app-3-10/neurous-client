...
- 퀴즈 진입 시 하루 1회 난이도 평가, 최근 20개 분석 기반 난이도 변경 제안
...
제안 조건을 충족한 분기에서 서버 평가 제출 전에 `return`해 해당 평가와 오늘 제출 날짜가 누락되는 문제가 있었다. 평가 서버 제출을 가장 먼저 완료하고, 성공한 평가만 로컬 이력에 저장·분석하도록 순서를 변경했다. 제안 수락 API도 성공 여부를 반환해 성공한 경우에만 모달 종료·변경 이벤트·토스트가 실행된다. 일일 제한은 기기 현지 날짜를 사용하며, 탈퇴·재가입 시 이전 계정의 평가 날짜와 이력을 초기화한다.

상세 아키텍처, Mermaid 시퀀스, 임계값과 QA 기준은 `docs/DIFFICULTY_FLOW.md` 참고.
...
| 4 | prefetch를 넣었지만 첫 요청이 느리면 1.5초 후 호출과 focus refetch가 진행 중인 동일 요청으로 합쳐져 후속 갱신이 사라짐 | **2026-08-31**: 첫 prefetch 완료 뒤 1.5초를 기다려 두 번째 요청을 시작하도록 직렬화 |
...
2026-08-31 후속 점검에서는 "함수를 두 번 호출했는가"와 "네트워크 요청이 실제로 두 번 발생했는가"도 구분했다. React Query는 같은 query key의 진행 중 요청을 공유할 수 있으므로, 서버 반영 지연에 대비한 재조회는 첫 요청 완료 뒤 시작되도록 순서를 보장해야 한다.
